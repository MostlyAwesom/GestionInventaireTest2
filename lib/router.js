...
Router.map(function() {
    ...
    this.route('ComputerUpdate', {
        path:'computer-update/:_id',
        template: 'computerUpdateLayout',
        data: function(){
            var brands = Brands.find();
            var models = Models.find();
            var currentComputer = Computers.findOne({
                _id: this.param._id
            });
            

            return {
                //currentId: this.param._id,
                brands: brands,
                models: models
            };
        },
        waitOn: function(){
            return [Meteor.subscribe("specificComputer", this.param._id),
                    Meteor.subscribe("allBrands"),
                    Meteor.subscribe("allModels")];
        }
    });
});
